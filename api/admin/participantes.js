...
module.exports = async function handler(req, res) {
  ...
  try {
    const { status, body } = await httpsGet(
      `${SUPA_URL}/rest/v1/participantes?select=nome,email,telefone,social,cidade,data_nascimento,link_compartilhamento,modalidade,quantidade,numeros,created_at&order=created_at.asc`,
      hdrs
    );
